...
def test_fetch_servers_worker(qtbot):
    ...
    with patch("discord_mass_cleanup.get_guilds", return_value=[{"id": "1", "name": "Guild"}]):
        with qtbot.waitSignal(worker.result_signal, timeout=1000) as blocker:
            worker.start()
...
def test_fetch_friends_worker(qtbot):
    ...
    with patch("discord_mass_cleanup.get_friends", return_value=[{"id": "1", "user": {}}]):
        with qtbot.waitSignal(worker.result_signal, timeout=1000) as blocker:
            worker.start()
...
def test_fetch_blocked_worker(qtbot):
    ...
    with patch("discord_mass_cleanup.get_blocked_users", return_value=[{"id": "1", "user": {}}]):
        with qtbot.waitSignal(worker.result_signal, timeout=1000) as blocker:
            worker.start()
...
def test_batch_action_worker_aborts_on_401(qtbot):
    """Verify that a batch operation halts immediately if token is invalidated (HTTP 401)."""
    items = [{"id": str(i), "user": {"username": f"user_{i}"}} for i in range(10)]
    worker = RemoveFriendsWorker("token", items)

    with patch("discord_mass_cleanup.remove_friend") as mock_remove:
        mock_remove.side_effect = [(204, ""), (401, "Unauthorized")]
...
def test_read_notifs_worker_cancellation_emits_finished(qtbot):
    """Verify that cancelling ReadNotifsWorker emits finished_signal so UI resets."""
    worker = ReadNotifsWorker("fake_token")
    mock_grouped = {"Server1": ["101", "102"]}
    with patch("discord_mass_cleanup._get_read_states", return_value=mock_grouped):
        with patch("discord_mass_cleanup._make_api_request", side_effect=dmc.RequestCancelled("Cancelled")):
            with qtbot.waitSignal(worker.finished_signal, timeout=1000) as blocker:
                worker.start()
