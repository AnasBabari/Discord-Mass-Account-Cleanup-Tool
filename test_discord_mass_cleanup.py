...
def test_get_guilds_success(mock_responses):
    ...
    mock_responses.add(
        ...
        json=page2,
        status=200,
        match=[responses.matchers.query_param_matcher({"limit": "200", "after": "199"})]
    )
...
def test_leave_guild_success(mock_responses):
    mock_responses.add(responses.DELETE, f"{BASE_URL}/users/@me/guilds/123", body="", status=204)
    assert dmc.leave_guild("token", "123") == (204, "")

@patch('time.sleep', return_value=None)
def test_leave_guild_429(mock_sleep, mock_responses):
    mock_responses.add(responses.DELETE, f"{BASE_URL}/users/@me/guilds/123", json={"retry_after": 0.5}, status=429)
    mock_responses.add(responses.DELETE, f"{BASE_URL}/users/@me/guilds/123", body="", status=204)
    assert dmc.leave_guild("token", "123") == (204, "")
    mock_sleep.assert_called_once_with(0.5)
...
def test_remove_friend_success(mock_responses):
    mock_responses.add(responses.DELETE, f"{BASE_URL}/users/@me/relationships/123", body="", status=204)
    assert dmc.remove_friend("token", "123") == (204, "")

@patch('time.sleep', return_value=None)
def test_remove_friend_429(mock_sleep, mock_responses):
    mock_responses.add(responses.DELETE, f"{BASE_URL}/users/@me/relationships/123", json={"retry_after": 0.5}, status=429)
    mock_responses.add(responses.DELETE, f"{BASE_URL}/users/@me/relationships/123", body="", status=204)
    assert dmc.remove_friend("token", "123") == (204, "")
    mock_sleep.assert_called_once_with(0.5)
...
def test_mass_leave_servers_success(mock_leave_guild, mock_get_guilds, mock_input, capsys):
...
@patch("discord_mass_cleanup.get_guilds")
@patch("discord_mass_cleanup.leave_guild")
def test_mass_leave_servers_all_and_cancel_confirm(mock_leave_guild, mock_get_guilds, mock_input, capsys):
    mock_get_guilds.return_value = [{"id": "1", "name": "Leavable 1"}]
...
def test_mass_remove_friends_full(mock_remove_friend, mock_get_friends, mock_input, capsys):
...
@patch("discord_mass_cleanup.get_friends")
@patch("discord_mass_cleanup.remove_friend")
def test_mass_remove_friends_cancel_confirm(mock_remove, mock_get_friends, mock_input, capsys):
    mock_get_friends.return_value = [{"id": "1", "user": {"username": "user1"}}]
    mock_input.side_effect = ["all", "no"]
    dmc.mass_remove_friends("token")
    captured = capsys.readouterr().out
...
def test_mass_mark_read_full(mock_mark_read, mock_get_dms, mock_input, capsys):
    mock_get_dms.return_value = [
        ...
        {"id": "3", "last_message_id": "300", "recipients": [{"username": "u1"}, {"username": "u2"}]},
...
def test_mass_mark_read_cancel(mock_get_dms, mock_input, capsys):
    mock_get_dms.return_value = [{"id": "1", "last_message_id": "100", "name": "Test Group"}]
